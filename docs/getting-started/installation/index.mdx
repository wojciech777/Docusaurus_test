...
| --- | --- | --- | --- |
| [Docker Compose](docker.mdx) | `dev` environment, workshops, demos, integration tests | 20-40 min | Community, Business, Enterprise |
| [Helm](kubernetes/helm.mdx) | `staging` and `prod` in an existing cluster, GitOps-managed deployments | 1-2 h | Community, Business, Enterprise |
| [Operator](kubernetes/operator.mdx) | `prod` with multiple instances, automatic migrations and backups | 1-3 h | Enterprise only |
| [System packages](bare-metal.mdx) | physical or virtual machines without containers, regulated environments | 2-4 h | Business, Enterprise |
...
- Working external dependencies: PostgreSQL 15, Redis 7, Kafka 3.6, S3/MinIO, Keycloak 24. See [Requirements](../requirements.mdx).
...
- The chosen profile: `dev`, `staging` or `prod`. See [Configuration profiles](../configuration/profiles.mdx).
...
<details>
<summary>Verification after every method</summary>
...
- [Install with Docker](docker.mdx)
- [Deploy on Kubernetes](kubernetes/index.mdx)
- [Install on a host](bare-metal.mdx)

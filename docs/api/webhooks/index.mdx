...
```json title="201 Created response" {3,9}
{
...
| `events[]` | array | yes | Event types or patterns such as `order.*`. At least one entry. See [Event types](event-types.mdx). |
...
The `secret` field appears in the `201 Created` response and never again. `GET /v1/webhooks/{id}` returns only the last four characters. If you lose the value, you cannot read it back: the only way forward is the rotation procedure described in [Signature verification](signature-verification.mdx).
:::
...
| --- | --- | --- | --- |
| List endpoints | `GET /v1/webhooks` | `webhooks:write` | Cursor pagination, `limit` defaults to 25 and maxes out at 100. See [Pagination, filtering, and sorting](../rest/pagination.mdx). |
...
| `X-Orion-Signature` | `t=1778490764,v2=...` | HMAC signature over the raw body. See [Signature verification](signature-verification.mdx). |
...
- **Retries.** Failed deliveries are retried with exponential backoff and jitter. The schedule, the classification of transient and permanent errors, and the circuit breaker are documented in [Retries and idempotency](../../guides/events/retries.mdx).
...
- **Dead letter queue.** Deliveries that exhaust all attempts land in `orion.events.v1.dlq` and increment the `orion_events_dlq_total` counter, which is wired to an alert described on the [Alerts](../../operations/monitoring/alerts.mdx) page.
...
The body is the event envelope documented in [Event types](event-types.mdx), with the schema selected by the endpoint's `api_revision`.
...
The same view is available in `orion-console`, together with the response body of the last failed attempt. Delivery level troubleshooting uses the `X-Orion-Delivery` identifier and the log search described in the [Alerts](../../operations/monitoring/alerts.mdx) page runbooks.
...
- [ ] The `whsec_...` secret is loaded from a secret store, not from the repository. See [Operational security](../../operations/security.mdx).
...
Outgoing deliveries do not consume the tenant's inbound request limit, as noted in [Rate limits](../rest/rate-limits.mdx).

## See also

- [Event types](event-types.mdx)
- [Signature verification](signature-verification.mdx)
- [Retries and idempotency](../../guides/events/retries.mdx)
- [Event processing](../../guides/events/index.mdx)
- [REST API basics](../rest/index.mdx)

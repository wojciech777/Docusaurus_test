...
recognized a duplicate. `orion-worker` treats such a delivery as <mark>successful</mark>
...
```python title="consumer.py" {8,15,20}
import random, time
import psycopg
...
```java title="EventConsumer.java" {10,16}

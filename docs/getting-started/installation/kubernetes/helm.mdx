...
```yaml title="values.yaml" {2,6,20,21,39}
global:
  profile: prod
  image:
...
Schema migrations are forward-only. After a release rollback, images from the older version may encounter a schema newer than expected and fail with `ORN-5003`. Rolling back across minor versions requires restoring the database from a backup, see [Backups](../../../operations/backups.mdx).
:::
...
<details>
<summary>Installing from an archive file</summary>
...
- [Deploy on Kubernetes](index.mdx)
- [Install with the operator](operator.mdx)
- [Environment variables](../../configuration/environment-variables.mdx)

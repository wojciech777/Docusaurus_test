...
- Kubernetes operator with the `OrionCluster` resource in version `v1alpha2`:
  ...
  backups. See [Install with the operator](getting-started/installation/kubernetes/operator.mdx).
- Tenant isolation based on RLS policies in PostgreSQL, plus the
  `X-Orion-Tenant` header. See [Multi-tenancy](guides/multi-tenancy.mdx).
...
- The `v2` webhook signature (HMAC-SHA256) is the default, with a timestamp
  tolerance of 300 s. See [Signature verification](api/webhooks/signature-verification.mdx).
...
- Rate limits are counted in a sliding window with a burst of 100 instead of a
  fixed window. See [Rate limits](api/rest/rate-limits.mdx).
...
- The `ORION_LEGACY_*` variables. The mapping to the new names is described in
  the [migration guide](guides/migrations/from-3-8-to-4-2.mdx).
...
[migration to 4.2 LTS](guides/migrations/from-3-8-to-4-2.mdx).
:::
...
- [Migrations and upgrades](guides/migrations/index.mdx)
- [Migrate from 3.8 LTS to 4.2 LTS](guides/migrations/from-3-8-to-4-2.mdx)

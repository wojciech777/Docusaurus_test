...
```yaml title="orion.yaml" {6,7,11,14}
events:
...
The equivalent environment variables use the `ORION_` prefix, for example `ORION_EVENTS_CONSUMER_GROUP_ID`. The full list is documented in [environment variables](../../getting-started/configuration/environment-variables.mdx).
...
The procedure for clearing a backlog is described in the [queue backlog runbook](../../operations/runbooks/queue-backlog.mdx).
...
<details>
<summary>How you know the DLQ is growing</summary>
...
which <mark>breaks the ordering guarantee</mark> for orders in flight. Perform this operation only
...
- [Event processing](index.mdx)
- [Retries and idempotency](retries.mdx)
- [Metrics](../../operations/monitoring/metrics.mdx)
- [Queue backlog](../../operations/runbooks/queue-backlog.mdx)

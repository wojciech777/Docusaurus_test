...
<Tabs>
<TabItem value="ubuntu" label="Ubuntu 22.04 / 24.04 (.deb)">
...
<TabItem value="rhel" label="RHEL 9 (.rpm)">
...
```ini title="/usr/lib/systemd/system/orion-core.service" {10,11,21}
[Unit]
...
Schema migrations are irreversible. Before every upgrade, take a full dump of the `orion` database and a copy of the `/etc/orion` directory following the procedure in [Backups](../../operations/backups.mdx). Without a backup there is no way back to the previous version.
:::

1. Review the list of changes for the target release in the [Changelog](../../changelog.mdx).
...
- [Backups](../../operations/backups.mdx)

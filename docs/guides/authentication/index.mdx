...
| Page | Contents |
| --- | --- |
| [OAuth2 client credentials](oauth2.mdx) | client registration, token request, JWT structure, secret rotation |
| [API keys](api-keys.mdx) | key format, creation, restriction, rotation without downtime |
| [Roles and permissions](roles-and-permissions.mdx) | RBAC model, built-in roles, permission matrix, audit |

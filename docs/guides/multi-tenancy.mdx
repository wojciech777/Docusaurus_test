...
```sql title="RLS policy for core.orders" {4,9,10}
...
returns data for <mark>all tenants</mark>. For analysis, use only the `orion_readonly` role,
...
Limits are raised by filing a request at `https://support.nimbus.example.com`. Exceeding the rate limit returns `ORN-3001` with the `X-RateLimit-Reset` header; the details are described in [rate limits](../api/rest/rate-limits.mdx).
...
<details>
<summary>Why not a topic per tenant</summary>
...
- [Roles and permissions](authentication/roles-and-permissions.mdx)
- [Event processing](events/index.mdx)

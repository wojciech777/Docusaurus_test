...
The buyer may close the tab before returning. The final state is confirmed by the `payment.authorized` or `payment.failed` event. See [Signature verification](../../webhooks/signature-verification.mdx).
:::

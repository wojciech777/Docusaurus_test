...
2. **Profile defaults** - `dev`, `staging` or `prod`, see [Configuration profiles](profiles.mdx).
...
4. **`ORION_*` environment variables** - see [Environment variables](environment-variables.mdx).
...
```yaml title="/etc/orion/orion.yaml" {1,6,22,34}
profile: prod
...
| `features.*` | yes | console | see [Configuration profiles](profiles.mdx) |
...
- [Configuration profiles](profiles.mdx)

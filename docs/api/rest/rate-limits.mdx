...
The most common cause of `ORN-3001` reports is a missing OAuth2 token cache. See [OAuth2](../../guides/authentication/oauth2.mdx).
:::
...
```http title="429 Too Many Requests response" {2,5}
...
- The `orion_http_requests_total` metric with the `endpoint`, `status`, and `tenant` labels lets you build an alert on the share of `429` responses. See [Metrics](../../operations/monitoring/metrics.mdx) for details.
- Recommended alert threshold: a share of `429` above 1 percent of requests over a 5 minute period.

## Raising a limit

A request to raise a limit requires the `ten_...` tenant identifier, the API key prefix, the target limit value, and a volume justification (average and peak requests per minute). A decision is issued within 3 business days. Form and contact channels: [Support](../../support.mdx).

## See also

- [Error codes](error-codes.mdx)
- [REST API basics](index.mdx)
- [Scaling](../../operations/scaling.mdx)
- [Alerts](../../operations/monitoring/alerts.mdx)

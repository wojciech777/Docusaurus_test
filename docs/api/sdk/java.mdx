...
```java title="OrionConfig.java" {9,10,11}
import com.nimbus.orion.OrionClient;
import com.nimbus.orion.OrionClientBuilder;
...
```java title="CheckoutService.java" {24,25}
import com.nimbus.orion.OrionClient;
...
:::note[Page limit]
The default `limit` is 25 and the maximum is 100. `autoPaging()` performs as many requests as needed; with wide time ranges it is worth narrowing the filters so as not to exhaust the limit described in [Rate limits](../rest/rate-limits.mdx).
:::
...
```java title="ErrorHandlingExample.java" {12,15}
...
```java title="OrionWebhookController.java" {20,23,24}

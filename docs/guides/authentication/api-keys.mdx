...
| --- | --- |
| Backend service serving customer traffic | [OAuth2](oauth2.mdx) |
| Cron job exporting orders once a day | `ok_live_...` key |
| Prototype and manual testing | `ok_test_...` key |
| Integration that needs different scopes on different paths | [OAuth2](oauth2.mdx) |
...
4. Roll the deployment out to the remaining instances. Keep the overlap period at <mark>7 days</mark>.
...
A production key is subject to a limit of 600 requests/min with a burst of 100; a sandbox key is limited to 60 requests/min. Responses include `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset`. The algorithm and the behavior on exceeding the limit are described in [rate limits](../../api/rest/rate-limits.mdx).

## See also

- [OAuth2 client credentials](oauth2.mdx)

...
The `OrionCertExpiring` alert with its 14-day threshold is described on the [alerts page](monitoring/alerts.mdx).
...
```yaml title="externalsecret-orion.yaml" {14,15,19}
...
Always rotate API keys and the webhook signing key with an overlap period. During that time signature verification accepts two values, see [signature verification](../api/webhooks/signature-verification.mdx).
:::
...
Audit records include `request_id`, which lets you join an entry with application logs following the rules described in [logs](monitoring/logs.mdx).
...
The history of patch releases is kept in the [changelog](../changelog.mdx).

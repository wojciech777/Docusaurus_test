...
<details>
<summary>Token payload preview</summary>
...
memory and refresh it when fewer than <mark>300 seconds</mark> remain until `exp`.
:::

```python title="Token cache with a safety margin" {6,7}

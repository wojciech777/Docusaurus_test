...
```yaml title="compose.yaml" {9,10,33,34}
name: orion
...
Add `.env` to `.gitignore`. The `ORION_ENCRYPTION_KEY` and `ORION_WEBHOOK_SIGNING_SECRET` values from this example are meant for the local environment only. Secret management rules are described in [Security](../../operations/security.mdx).
:::
...
The `-v` flag deletes the `pgdata`, `kafkadata` and `miniodata` volumes together with the orders, the ledger and the events. Take a dump first, as described in [Backups](../../operations/backups.mdx).
:::
